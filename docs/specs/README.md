...
| `AUTH_SERVICE_LEGACY_SURFACE_SHUTDOWN_V1` | program | proposed | none | Legacy 鉴权面硬切 shutdown Program（parent authority：accepted `MINIMAL_AUTH_FOUNDATION_V2`，Contract `1.3.0`）：Human Principal administration（User create / claim / status / enable / disable）已委托给 `AUTH_SERVICE_HUMAN_PRINCIPAL_ADMINISTRATION_V1`（PR #15），password reset 委托给独立 credential-only child；Program Spec，不直接授权实现，每个 implementation Child 需独立 accepted Child Spec |
