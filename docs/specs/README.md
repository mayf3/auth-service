...
| `AUTH_SERVICE_DEVELOPMENT_GOVERNANCE_ADOPTION_V1` | invariant | superseded | none | 精确 vendoring 并采用共享开发治理；不改变产品行为。已由 `AUTH_SERVICE_DEVELOPMENT_GOVERNANCE_ADOPTION_V2` whole-Spec supersede；保留为历史 accepted authority。 |
| `AUTH_SERVICE_DEVELOPMENT_GOVERNANCE_ADOPTION_V2` | invariant | accepted | none | Governance v1.0.0 whole-authority successor（current）：三轴 Authority/Plan/Assurance 路由、Execution Mandate、route validator 与 DONE_WHEN/EXPANSION_TRIGGER stop control；仅治理采用，不改变产品、运行或生产。acceptance transaction 已原子完成——V2 accepted/none，V1 superseded 并回填 backlink（V1 normative body 字节保留） |
...
| `AUTH_SERVICE_FORUM_MODERATOR_GRANT_SUPPLY_BUNDLE_RETARGET_V1` | implementation | superseded | contracts（仅三文件：vehicle script + 其测试 + harness 前置常量；production apply 仍由父 Spec CTR-FMG-016 另行授权） | 父 Spec `AUTH_SERVICE_FORUM_MODERATOR_GRANT_SUPPLY_V1` 的 focused mechanical amendment（非 successor）：车辆 Bundle pin `1.7.0`→`1.8.0`（现行役 Bundle，CCR-EAPR-001 带 registry svc-forum target entry）+ CTR-FMG-008 矩阵恰增一条合法前置态——Bundle redeploy 合法产物 Audience@exact-target + Grant@source + 无 FMG audit → `APPLY UPDATE_GRANT_ONLY`（audience 行字节不动、grant v1→v2、governed audit 绑定 Grant-only plan digest；三个 canonical digest 两两相异；反向 mixed 仍硬 conflict）；其余父语义（identity tuple、scope 集、13-field envelope、pre-connection 拒绝、advisory lock、Serializable）字节不变 |
| `AUTH_SERVICE_FORUM_MODERATOR_GRANT_SUPPLY_BUNDLE_RETARGET_V2` | implementation | accepted (BASE `1de2311e`, reviewed/accepted Head `81383523`, DIFF_DIGEST `85dea49e…`, independent audit ACCEPT 0 blockers, Owner acceptance PR #63 issuecomment-5586354195 @ 2026-09-08T14:00:41Z preceding lifecycle commit; supersedes RETARGET_V1 pin authority) | contracts（仅三文件：vehicle script + 其测试 + harness 前置常量；另加本 Spec 文档 + README 索引行） | 前一 mechanical amendment 的机械重绑继任（非语义变更）：车辆 Bundle pin `1.8.0`→`1.11.0`（BASE `1de2311e` 现行役 Bundle；svc-forum registry entry 与 1.8.0 代 canonical 字节一致，sha256 `14763850c3b5b1e8…`），使车辆可对现役 Bundle 执行 plan/conformance/verify；identity/scope/事务原子性/审计封装/CTR-FMG-016 生产授权边界全部字节不变 |
| `AUTH_SERVICE_FORUM_MODERATOR_GRANT_SUPPLY_PRESTATE_REBASELINE_V1` | implementation | proposed | contracts（FMG 车辆 + 其测试 + 本 Spec 文档 + README 行） | FMG 前置所有权收窄 amendment（真实语义修正，非 mechanical）：FMG 只拥有 identity tuple + svc-forum Audience 行 + svc-forum Grant 行 + governed audit；同 Client 一切其他 Grant 行为 bystander（0..N，进 invariant digest 与完整 before/after 快照，事务字节保全，任何 bystander 漂移经 prestate digest fail-closed）；解除 total-row-count==2 与 CTR-FMG-005 svc-workflow `[workflow.read]@v1` exact pin 作为 APPLY gate（WORKFLOW_GRANT_MUTATION 仍 FORBIDDEN，EXACT_STATE_REQUIRED=NO）；同时冻结 CTR-FMG-016 apply activation closure（14-field exact descriptor、pre-connection 校验、implementation commit=clean HEAD、bundle file digest、plan/prestate digest 事务内复验、ATTEMPTS_MAX=1、sanitized receipt）；当前生产 5 行 prestate 仅作 acceptance fixture（T2），T3 证明任意新增 bystander 不再使车辆失效；WRITE_SET_CHANGED=NO，PERMISSION_TARGET_CHANGED=NO |
...
| `AUTH_SERVICE_HR_AGENT_SESSION_SEND_GRANT_V1` | implementation | superseded (2026-09-05 by `AUTH_SERVICE_HR_AGENT_SESSION_SEND_GRANT_V2` whole-Spec subject successor；reviewed head 9b3b4bd…) | contracts（生产 apply 另受 shared mutation slot + controlled runbook + 原生特权 gate 约束） | 独立的最小 HR orchestration send Grant supply：exact Principal `bc970ced…`(hr-agent) 的唯一 active MachineClient × `agent-session-messaging` × `[agent.session.send]` v1 恒久 operational Grant；tombstone-only 回滚 + 同事务 nonce audit + bounded reason（`HR_WORKFLOW_ORCHESTRATION_AGENT_SESSION_SEND_ACTIVATION`）；零 scheduler.admin / agent_wake / 跨 Agent mutation / credential 变更；不复用不改 supersede efficiency 的 operational Grant |
| `AUTH_SERVICE_HR_AGENT_SESSION_SEND_GRANT_V2` | implementation | accepted (reviewed head 3a5e01e…, Owner batched exact-head acceptance 2026-09-05) | contracts（生产 apply 另受 shared mutation slot + controlled runbook + 原生特权 gate 约束） | V1 的 whole-Spec subject successor：唯一语义修正=HR 业务主体 bc970ced…(hr-agent, legacy) → dc702687…(agt_hr-agent, 现行正式 HR 业务身份) + legacy 主体 MUST NOT 否定条款；capability 语义字节保留（最小 send Grant、零 scheduler.admin/agent_wake/跨 Agent mutation/credential/admin）；原子接受 V2 accepted + V1 superseded 回填 |
| `AUTH_SERVICE_EXACT_AGENT_PRINCIPAL_RESOLUTION_V1` | implementation | superseded (2026-09-05 by `AUTH_SERVICE_EXACT_AGENT_PRINCIPAL_RESOLUTION_V2` whole-Spec subject successor；reviewed head 0359575…) | contracts（生产 apply 另受 shared mutation slot + controlled runbook 约束，`production_apply_authority: conditional_controlled_operation`） | 独立 bounded 读操作：`GET /api/v1/agent-principals/:principal_id/agent`（exact UUID → active AGENT Principal 的 canonical agentId；fail-closed 404/409/422/500/504 全族；正反向 duplicate 双行界；无 name/external-ref inference）+ additive child CCR entry `agent-principal-resolution` × `auth.agent.resolve`（machine-only，bundle additive minor）+ HR 唯一 tuple 读 Grant（机械绑定 exact active client）；不改 supersede external-ref discovery Spec；下游 = dsh AGENT_CORE_EXACT_PRINCIPAL_AGENT_RESOLUTION_V1 depends_on pin；零 credential/secret 披露 |
| `AUTH_SERVICE_EXACT_AGENT_PRINCIPAL_RESOLUTION_V2` | implementation | accepted (reviewed head 87beb77…, Owner batched exact-head acceptance 2026-09-05) | contracts（生产 apply 另受 shared mutation slot + controlled runbook 约束，`production_apply_authority: conditional_controlled_operation`） | V1 的 whole-Spec subject successor：唯一语义修正=HR 读 Grant 接收者 bc970ced…(legacy) → dc702687…(agt_hr-agent) + legacy MUST NOT 条款；subject-generic resolution 契约（registration entry、route、error taxonomy、lifecycle）字节保留；下游 = dsh AGENT_CORE_EXACT_PRINCIPAL_AGENT_RESOLUTION_V2 pin |
...
| `LIFE_WORKBENCH_AGENT_ACCESS_POLICY_V1` | policy | accepted | none（三层访问政策：ACCESS_ENTITLEMENT / GRANT_PROVISIONING_PRECONDITIONS / RUNTIME_READINESS；`ELIGIBLE_BUT_IDENTITY_NOT_READY` 与 `AUTHORIZED_BUT_RUNTIME_NOT_READY` 语义分离；PILOT_ROSTER != FINAL_ELIGIBILITY_RULE；acceptance 不授权任何 rollout） | 生命工作台 Agent 基础访问（workbench.read + workbench.propose）的产品规则权威；membership 权威 = accepted `AGENT_CORE_AGENT_CREDENTIAL_PROVISIONING_V1` 的 canonical Agent Definition config。 |
