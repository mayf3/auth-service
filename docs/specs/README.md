...
| `AUTH_SERVICE_AGENT_CORE_NOTIFICATION_INGRESS_SERVICE_CREDENTIAL_GRANT_V1` | implementation | proposed | none（`production_apply_authority: none`） | PR #28 未接受候选的 focused amendment：严格分离 Audience 注册与 Client/Grant 供应；冻结两个 exact Client ID、exact 三文件闭包、plan/apply/verify、exact-rerun NOOP、drift fail-closed、非目标 Client/Grant digest 不变、secret 零披露、Phase B 完整回滚与不完整回滚 fail-loud；当前 integrated WIP runner 保持 BLOCKED/unchanged；docs-only，零生产效果。 |
