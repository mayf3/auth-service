...
| `LIFE_WORKBENCH_AGENT_ACCESS_POLICY_V1` | policy | proposed | none（三层访问政策：ACCESS_ENTITLEMENT / GRANT_PROVISIONING_PRECONDITIONS / RUNTIME_READINESS；`ELIGIBLE_BUT_IDENTITY_NOT_READY` 与 `AUTHORIZED_BUT_RUNTIME_NOT_READY` 语义分离；PILOT_ROSTER != FINAL_ELIGIBILITY_RULE；acceptance 不授权任何 rollout） | 生命工作台 Agent 基础访问（workbench.read + workbench.propose）的产品规则权威；membership 权威 = accepted `AGENT_CORE_AGENT_CREDENTIAL_PROVISIONING_V1` 的 canonical Agent Definition config。 |
| `LIFE_WORKBENCH_AUDIENCE_CCR_V1` | implementation | proposed | contracts（仅 CTR-LW-001 冻结的三文件闭包；`production_apply_authority: none`） | 注册 `life-workbench` Audience（machine-only、agent-profile、恰 `workbench.propose` + `workbench.read` 两 scope、no-delegation）的 bounded child CCR：registry 1.8.0→1.9.0 append-only（九现有条目 byte 不变）、离线 reconcile、正负 conformance、§5 冻结唯一 rollback end-state（status=disabled，不 DELETE）；不创建 Principal/Client/Grant、零生产效果。 |
