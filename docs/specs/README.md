...
| `AUTH_SERVICE_SCHEDULER_AUDIENCE_CCR_V1` | implementation | proposed | none | 注册 `scheduler` Audience（machine-only、agent-profile、恰 `scheduler.admin` + `scheduler.audit` 两 scope）的 bounded child CCR：scope 语义锚点 = accepted AGENT_CORE_SCHEDULER_RUN_HISTORY_V1 R8（blob 双点一致）；manage-any 线格式/本地标签/global definition-read 一律不注册；16 文件 exact implementation closure（NI closure V2 同构）；版本判定延迟到实现时点并规避占用/预留/sibling pending CCR；下游 = dsh-agent-core scheduler 部署链 prerequisite（DAG ROOT，无下游 head pin；proposed PR #144 仅 context）；不创建 Principal/Client/Grant、不写 DB、零生产效果 |
