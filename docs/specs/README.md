...
| `AUTH_SERVICE_SCHEDULER_BUNDLE_1_7_DEPLOYMENT_V1` | implementation | proposed | contracts（accepted+merged 前 inactive） | exact `57258ec` scheduler-only bundle 1.7.0 production deployment child Authority：sealed snapshot、单一 scheduler AuthAudience backfill、native Owner gate、one restart、negative-only pre-Grant proof、equal-face rollback；排除 later-main forum delta 与一切 Grant |
| `AUTH_SERVICE_DAILY_AUTONOMY_OPERATIONAL_GRANTS_V1` | implementation | proposed | contracts（accepted+merged 前 inactive） | daily autonomy 的两项永久最小权限：仅 `agt_efficiency-agent` 既有 client；reactivate ASM temp tombstone 为 `agent.session.send` v2，并创建 `scheduler.admin` v1；明确排除 `scheduler.audit`、targets/fleet、aliases/wildcards、credential 变化，含精确补偿与 native Owner gate |
