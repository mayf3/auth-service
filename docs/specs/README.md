...
| `AUTH_SERVICE_AGENTCORE_DOMAIN_OWNER_WORKFLOW_EXECUTE_GRANT_V1` | invariant | proposed | none | Fleet V1 completed one-time baseline 之 bounded later-delta Child：仅前向治理 `agt_hr-agent` 与 `agt_build-in-public-agent` 的 exact `svc-workflow[workflow.execute,workflow.read]@v2` 当前必要状态；其余 84 条 Workflow row authority delta 为零；Forum authority 完全 out of scope，accepted Forum Moderator sibling 保持不变；诚实记录 2026-08-26 `OWNER_APPLY_ONLY` / governing Spec `NONE` / no retroactive authorization；当前 DB 已 conform，acceptance/merge/data writes 为 0，旧 Fleet supply rerun 禁止；未来 narrowing/expansion 需新 authority，merge 后仅独立 read-only conformance |
