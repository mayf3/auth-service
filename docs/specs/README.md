...
| `AUTH_SERVICE_HR_AGENT_SESSION_SEND_GRANT_V1` | implementation | accepted (reviewed head 9b3b4bd…, Owner batched exact-head acceptance 2026-09-05) | contracts（生产 apply 另受 shared mutation slot + controlled runbook + 原生特权 gate 约束） | 独立的最小 HR orchestration send Grant supply：exact Principal `bc970ced…`(hr-agent) 的唯一 active MachineClient × `agent-session-messaging` × `[agent.session.send]` v1 恒久 operational Grant；tombstone-only 回滚 + 同事务 nonce audit + bounded reason（`HR_WORKFLOW_ORCHESTRATION_AGENT_SESSION_SEND_ACTIVATION`）；零 scheduler.admin / agent_wake / 跨 Agent mutation / credential 变更；不复用不改 supersede efficiency 的 operational Grant |
| `AUTH_SERVICE_EXACT_AGENT_PRINCIPAL_RESOLUTION_V1` | implementation | accepted (reviewed head 0359575…, Owner batched exact-head acceptance 2026-09-05) | contracts（生产 apply 另受 shared mutation slot + controlled runbook 约束，`production_apply_authority: conditional_controlled_operation`） | 独立 bounded 读操作：`GET /api/v1/agent-principals/:principal_id/agent`（exact UUID → active AGENT Principal 的 canonical agentId；fail-closed 404/409/422/500/504 全族；正反向 duplicate 双行界；无 name/external-ref inference）+ additive child CCR entry `agent-principal-resolution` × `auth.agent.resolve`（machine-only，bundle additive minor）+ HR 唯一 tuple 读 Grant（机械绑定 exact active client）；不改 supersede external-ref discovery Spec；下游 = dsh AGENT_CORE_EXACT_PRINCIPAL_AGENT_RESOLUTION_V1 depends_on pin；零 credential/secret 披露 |
