...
| `AUTH_SERVICE_AGENTCORE_TRUSTED_FLEET_WORKFLOW_GRANT_NARROWING_V1` | implementation | proposed | none | exact two-row `svc-workflow` Grant narrowing：`workflow.execute + workflow.read → workflow.read`、version `2 → 3`、both-row serializable atomicity、same-transaction audits、exact rerun NOOP；implementation 与 production apply 均未授权 |
