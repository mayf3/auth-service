...
async function seed(options: SeedOptions = {}): Promise<void> {
  ...
  await prisma.machinePrincipal.create({ data: {
    id: TARGET_PRINCIPAL_ID, principalType: 'agent', status: 'active', createdAt: CREATED, updatedAt: CREATED,
  } });
  await prisma.machinePrincipal.create({ data: {
...
for (const scenario of [
  ...
  test(`conflict fail-closed: ${scenario.name}`, async () => {
    ...
    assert.equal((await counts()).grants, expectedGrants, 'grant rows unchanged');
  });
}
