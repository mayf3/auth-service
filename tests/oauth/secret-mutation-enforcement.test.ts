...
describe('Secret-Mutation Enforcement Seam', () => {
  ...
  it('T5: direct raw UPDATE of secret_hash alone is REJECTED with zero mutation', async () => {
    if (!(await seamAvailable())) return;
  ...
  it('T6: p4-form ORM update of secretHash only is REJECTED (create-or-overwrite pattern)', async () => {
    if (!(await seamAvailable())) return;
    const before = await prisma.machineClient.findUnique({ where: { id: clientDbId } });
    await expectPermissionDenied(
  ...
  it('T7: raw UPDATE of secret_hash + rotated_at together is REJECTED outside the seam', async () => {
    if (!(await seamAvailable())) return;
    const before = await prisma.machineClient.findUnique({ where: { id: clientDbId } });
    await expectPermissionDenied(
  ...
  it('T8: canonical rotate changes generation exactly once, advances rotated_at/updated_at, writes receipt', async () => {
    if (!(await seamAvailable())) return;
    const before = await prisma.machineClient.findUnique({ where: { id: clientDbId } });
  ...
  it('A9-negative: preimage fingerprint mismatch is rejected with ZERO mutation', async () => {
    if (!(await seamAvailable())) return;
    const before = await prisma.machineClient.findUnique({ where: { id: clientDbId } });
  ...
  it('T10: replaying the same operation id is idempotent — no second secret change', async () => {
    if (!(await seamAvailable())) return;
  ...
  it('sanity: INSERT (CREATE) path still works for the application role', async () => {
    if (!(await seamAvailable())) return;
    const client = await createClient({
      agentId: `${TEST_PREFIX}-agent`,
      resources: ['svc-forum'],
  ...
  it('fixture principal is resolvable', async () => {
    if (!(await seamAvailable())) return;
