...
DATABASE='auth_stage_f_conformance'
...
cleanup() {
  ...
  if [[ -n "$CONTAINER_ID" ]]; then
    ...
    observed_label="$(/usr/local/bin/docker inspect --format '{{index .Config.Labels "com.mayf3.auth.stage-f-conformance"}}' "$CONTAINER_NAME" 2>/dev/null || true)"
  ...
  if [[ -n "$CONTAINER_ID" ]] && /usr/local/bin/docker inspect "$CONTAINER_ID" >/dev/null 2>&1; then
    ...
    remaining_label="$(/usr/local/bin/docker inspect --format '{{index .Config.Labels "com.mayf3.auth.stage-f-conformance"}}' "$CONTAINER_NAME" 2>/dev/null || true)"
  ...
  echo 'STAGE_F_CONFORMANCE_CONTAINER_ABSENT=true'
  exit "$ORIGINAL_EXIT"
}
...
CONTAINER_NAME="auth-stage-f-conformance-$(printf '%s' "$NONCE" | cut -c1-16)"

CONTAINER_ID="$(/usr/local/bin/docker run -d --rm \
  --name "$CONTAINER_NAME" \
  --label "com.mayf3.auth.stage-f-conformance=sha256:$NONCE_HASH" \
...
CREATE ROLE stage_f_runner LOGIN;
GRANT USAGE ON SCHEMA public TO stage_f_runner;
GRANT SELECT (id, client_id, machine_principal_id, external_ref, status) ON machine_clients TO stage_f_runner;
GRANT SELECT ON machine_principals, auth_audiences, trusted_proxies, delegation_grants TO stage_f_runner;
GRANT SELECT, INSERT, UPDATE, DELETE ON machine_access_grants, grant_change_audits TO stage_f_runner;
COMMIT;
SQL

LEGACY_PRIVILEGES="$(/usr/local/bin/docker exec "$CONTAINER_ID" psql -At -U postgres -d "$DATABASE" -c \
  "SELECT has_column_privilege('stage_f_runner','machine_clients','allowed_resources','SELECT')::text || ',' || has_column_privilege('stage_f_runner','machine_clients','allowed_scopes','SELECT')::text")"
[[ "$LEGACY_PRIVILEGES" == 'false,false' ]] || { echo 'FATAL: conformance runner can read legacy columns' >&2; exit 1; }
echo 'STAGE_F_LEGACY_COLUMN_SELECT_PRIVILEGES=false,false'
...
BEGIN
  BEGIN
    INSERT INTO grant_change_audits VALUES
      ('00000000-0000-4000-8000-000000000001','probe-source','BAD','op','approval','reason','mc_probe','replace',1,2,'{}','{}',now());
  ...
  BEGIN
    INSERT INTO grant_change_audits VALUES
      ('00000000-0000-4000-8000-000000000002','','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','op','approval','reason','mc_probe','replace',1,2,'{}','{}',now());
  ...
  BEGIN
    INSERT INTO grant_change_audits VALUES
      ('00000000-0000-4000-8000-000000000003','probe-reason','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','op','approval','','mc_probe','replace',1,2,'{}','{}',now());
  ...
  BEGIN
    INSERT INTO grant_change_audits VALUES
      ('00000000-0000-4000-8000-000000000004','probe-version','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','op','approval','reason','mc_probe','replace',1,0,'{}','{}',now());
  ...
  BEGIN
    INSERT INTO grant_change_audits VALUES
      ('00000000-0000-4000-8000-000000000005','probe-shape','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','op','approval','reason','mc_probe','replace',1,2,'{}',NULL,now());
...
END $$;
INSERT INTO grant_change_audits VALUES
  ('00000000-0000-4000-8000-000000000006','probe-trigger','aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','op','approval','reason','mc_probe','replace',1,2,'{}','{}',now());
DO $$ BEGIN
...
echo 'STAGE_F_AUDIT_CONTROLS_VERIFIED=true'
DESCRIPTOR="$(node -e '
const d={schema_version:1,container_id:process.argv[1],nonce:process.argv[2],host_port:Number(process.argv[3]),database:"auth_stage_f_conformance",audit_metadata:{migration_id:"stage-f-conformance-v1",source_git_commit:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",operator_id:"stage-f-conformance",approval_ref:"https://github.com/mayf3/auth-service/issues/1#issuecomment-1",reason:"isolated Stage F conformance"}}; process.stdout.write(JSON.stringify(d));
' "$CONTAINER_ID" "$NONCE" "$HOST_PORT")"

(
  cd "$REPO_DIR"
  env -u DATABASE_URL node --import tsx tests/oauth/supply-agentcore-canary-forum-grants-v1.test.ts \
    3< <(printf '%s' "$DESCRIPTOR")
)

echo 'STAGE_F_TEMP_DB_CONFORMANCE=PASS'
