...
// Stage F — FORUM_CANARY_GRANT_SUPPLY per AUTH_SERVICE_AGENTCORE_CANARY_GRANT_SUPPLY_V1.
// Mirrors scripts/supply-agentcore-canary-workflow-grants-v1.ts (Stage W) with the
// Stage F semantics frozen by the Spec: precondition is the exact Stage W
// end-state (workflow-only snapshot at audit-derived revision 1), the write is a
// `replace` to revision 2 adding only svc-forum[forum.read,forum.write], and apply
// is gated on the three §3 forum prerequisites in addition to Stage W-style
// migration evidence.
...
type StagePlan = { outcome: 'replace' | 'noop'; clients: PlannedClient[] };
...
const SCOPES = Object.freeze(['forum.read', 'forum.write']);
...
const DATABASE = 'auth_stage_f_conformance';
const API_HOST = 'api.github.com';
const API_PORT = 443;
const HEADERS = Object.freeze({
  'User-Agent': 'mayf3-auth-service-stage-f-v1',
...
const FORUM_CONSUMER_COMMIT = '1cccdd54554c0bde13572273401f19f294334e46';
const BUNDLE_CONTRACT_VERSION = '1.3.0';

function fail(message: string): never {
  throw new Error(`Stage F refused: ${message}`);
}
...
async function loadStagePlan(db: Db): Promise<StagePlan> {
  ...
  if (audienceRows.length !== 1) fail('svc-forum must resolve exactly once');
  ...
  if (audience.status !== 'active' || !audience.machineAccessEnabled
    ...
    fail('svc-forum is inactive, not machine/agent enabled, or does not register forum.read and forum.write');
  }
  ...
  for (const target of TARGETS) {
    ...
    if (client.trustedProxy !== null && client.trustedProxy.delegationGrants.length !== 0) {
      fail(`${target.agentId} has delegation grants outside the Stage F target snapshot`);
    }
    ...
    const workflowOnlySnapshot = completeSnapshot(client,
      workflowGrant ? [workflowGrant] : [], 1);
    const workflowForumSnapshot = completeSnapshot(client,
      [workflowGrant, forumGrant].filter((grant) => grant !== undefined), 2);

    // Exact Stage F end-state: workflow grant untouched at version 1, forum grant at
    // version 2, audit trail = Stage W create (null→1) + Stage F replace (1→2).
    const workflowRowExact = client.accessGrants.length === 2 && workflowGrant !== undefined
      && forumGrant !== undefined
      && workflowGrant.version === 1 && sameJson([...workflowGrant.scopes].sort(asciiCompare), WORKFLOW_SCOPES)
      && forumGrant.version === 2 && sameJson([...forumGrant.scopes].sort(asciiCompare), SCOPES);
    const stageFAuditExact = audits.length === 2
      && audits[1].changeType === 'replace'
      && audits[1].expectedGrantVersion === 1
      && audits[1].resultingGrantVersion === 2
      && audits[1].clientId === client.clientId
      && sameJson(audits[1].beforeValue, workflowOnlySnapshot)
      && sameJson(audits[1].afterValue, workflowForumSnapshot);
    if (workflowRowExact && stageFAuditExact
      && stageWAuditExact(audits, client.clientId, workflowOnlySnapshot, 2)) {
      planned.push({ target, internalClientId: client.id, publicClientId: client.clientId,
        principalId: client.machinePrincipalId, outcome: 'noop',
        beforeValue: workflowOnlySnapshot, afterValue: workflowForumSnapshot });
      continue;
    }

    // Stage F precondition: the exact Stage W end-state — one workflow-only grant row
    // at version 1 and exactly one create audit (null→1) with the complete snapshot.
    const preconditionGrants = client.accessGrants.length === 1 && workflowGrant !== undefined
      && workflowGrant.version === 1 && sameJson([...workflowGrant.scopes].sort(asciiCompare), WORKFLOW_SCOPES);
    if (!preconditionGrants || !stageWAuditExact(audits, client.clientId, workflowOnlySnapshot, 1)) {
      fail(`${target.agentId} existing grant set conflicts with Stage F precondition`);
    }
    const targetAfterValue = completeSnapshot(client,
      [workflowGrant, { audienceId: AUDIENCE, scopes: [...SCOPES], version: 2 }], 2);
    planned.push({ target, internalClientId: client.id, publicClientId: client.clientId,
      principalId: client.machinePrincipalId, outcome: 'replace',
      beforeValue: workflowOnlySnapshot, afterValue: targetAfterValue });
  }
  if (planned.some((item) => item.outcome !== planned[0].outcome)) {
    fail('canary clients are in mixed Stage F states');
  }
  return { outcome: planned[0].outcome, clients: planned };
}

function stageWAuditExact(audits: Array<{
  changeType: string; expectedGrantVersion: number | null; resultingGrantVersion: number;
  clientId: string; beforeValue: unknown; afterValue: unknown;
}>, clientId: string, workflowOnlySnapshot: Prisma.InputJsonObject, exactCount: number): boolean {
  return audits.length === exactCount && audits[0].changeType === 'create'
    && audits[0].expectedGrantVersion === null
    && audits[0].resultingGrantVersion === 1
    && audits[0].clientId === clientId
    && audits[0].beforeValue === null
    && sameJson(audits[0].afterValue, workflowOnlySnapshot);
}
...
function auditEnvelope(metadata: Metadata, client: PlannedClient, id: string, timestamp: Date): Record<string, unknown> {
  const envelope = {
    ...
    before_value: client.beforeValue,
...
async function applyStage(prisma: PrismaClient, metadata: Metadata): Promise<StagePlan> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.$executeRaw`SELECT pg_advisory_xact_lock(813_947_202)`;
    ...
    for (const client of plan.clients) {
      ...
      await tx.grantChangeAudit.create({ data: {
        ...
        beforeValue: client.beforeValue,
...
function report(plan: StagePlan, operation: string): void {
  process.stdout.write(`${JSON.stringify({
    stage: 'F', operation, outcome: plan.outcome,
    grants_created: plan.outcome === 'replace' ? 2 : 0,
    audits_created: plan.outcome === 'replace' ? 2 : 0,
...
async function commitSha(repo: 'dsh-agent-core' | 'auth-service', ref: string): Promise<string> {
  const path = `/repos/mayf3/${repo}/commits/${ref}`;
  const body = await requestJson(repo === 'auth-service' ? 'auth-commit' : 'commit', path, 2 * 1024 * 1024);
  return lowercaseHex(body.sha, 40, `commit ${ref}`);
}
async function requireAncestor(repo: 'dsh-agent-core' | 'auth-service', base: string, head: string): Promise<void> {
  const path = `/repos/mayf3/${repo}/compare/${base}...${head}`;
  const body = await requestJson(repo === 'auth-service' ? 'auth-compare' : 'compare', path, 16 * 1024 * 1024);
...
async function validateEvidence(commit: string, pathValue: string): Promise<Metadata> {
  ...
  const remoteMain = await commitSha('dsh-agent-core', 'main');
  if (await commitSha('dsh-agent-core', commit) !== commit) fail('evidence commit response mismatch');
  await requireAncestor('dsh-agent-core', commit, remoteMain);
  ...
  exactKeys(manifest, ['schema_version', 'identities', 'readiness', 'migration_review', 'approval', 'audit_metadata', 'forum_prerequisites'], 'manifest');
  if (manifest.schema_version !== 1 || !isObject(manifest.readiness)
      || !isObject(manifest.migration_review) || !isObject(manifest.approval) || !isObject(manifest.audit_metadata)
      || !isObject(manifest.forum_prerequisites)
      || !Array.isArray(manifest.identities) || manifest.identities.length !== 2) fail('manifest shape invalid');

  // §3 forum prerequisites — every one must hold before any svc-forum write.
  const forum = manifest.forum_prerequisites;
  const ccr = forum.audience_ccr;
  if (!isObject(ccr)) fail('forum_prerequisites.audience_ccr invalid');
  exactKeys(ccr, ['merged', 'merge_commit', 'spec', 'receipt'], 'forum_prerequisites.audience_ccr');
  if (ccr.merged !== true) fail('svc-forum audience CCR is not merged');
  exactText(ccr.spec, 'AUTH_SERVICE_SVC_FORUM_AUDIENCE_CCR_V1', 'forum_prerequisites.audience_ccr.spec');
  const ccrCommit = lowercaseHex(ccr.merge_commit, 40, 'forum_prerequisites.audience_ccr.merge_commit');
  if (await commitSha('auth-service', ccrCommit) !== ccrCommit) fail('forum CCR merge commit mismatch');
  const authMain = await commitSha('auth-service', 'main');
  await requireAncestor('auth-service', ccrCommit, authMain);

  const consumer = forum.consumer_migration_review;
  if (!isObject(consumer)) fail('forum_prerequisites.consumer_migration_review invalid');
  exactKeys(consumer, ['passed', 'consumer_commit', 'receipt'], 'forum_prerequisites.consumer_migration_review');
  if (consumer.passed !== true
      || exactText(consumer.consumer_commit, FORUM_CONSUMER_COMMIT, 'forum_prerequisites.consumer_migration_review.consumer_commit') !== FORUM_CONSUMER_COMMIT) {
    fail('svc-forum consumer migration review prerequisite does not hold');
  }

  const bundle = forum.contract_bundle;
  if (!isObject(bundle)) fail('forum_prerequisites.contract_bundle invalid');
  exactKeys(bundle, ['updated_and_validated', 'contract_version', 'receipt'], 'forum_prerequisites.contract_bundle');
  if (bundle.updated_and_validated !== true
      || exactText(bundle.contract_version, BUNDLE_CONTRACT_VERSION, 'forum_prerequisites.contract_bundle.contract_version') !== BUNDLE_CONTRACT_VERSION) {
    fail('contract bundle prerequisite does not hold');
  }
  ...
  if (reviewReceipt.receipt_type !== 'auth_service_stage_f_migration_review' || reviewReceipt.auth_repository !== 'mayf3/auth-service'
  ...
  if (approvalReceipt.receipt_type !== 'auth_service_stage_f_operator_approval' || approvalReceipt.auth_repository !== 'mayf3/auth-service'
      || approvalReceipt.status !== 'APPROVED' || approvalReceipt.operator_id !== operatorId || approvalReceipt.approval_ref !== approvalRef
      || approvalReceipt.reviewed_source_git_commit !== reviewed || approvalReceipt.authorized_stage !== 'STAGE_F'
  ...
  const ccrReceipt = await loadReceipt(receiptLink(ccr.receipt, 'forum_prerequisites.audience_ccr.receipt'), commit, 'forum CCR receipt');
  baseReceipt(ccrReceipt, ['schema_version', 'repository', 'recorded_at', 'receipt_type', 'status', 'spec_id', 'merge_commit', 'auth_repository'], 'forum CCR receipt');
  if (ccrReceipt.receipt_type !== 'svc_forum_audience_ccr_acceptance' || ccrReceipt.status !== 'MERGED'
      || ccrReceipt.spec_id !== ccr.spec || ccrReceipt.merge_commit !== ccrCommit
      || ccrReceipt.auth_repository !== 'mayf3/auth-service') fail('forum CCR receipt invalid');
  const consumerReceipt = await loadReceipt(receiptLink(consumer.receipt, 'forum_prerequisites.consumer_migration_review.receipt'), commit, 'consumer review receipt');
  baseReceipt(consumerReceipt, ['schema_version', 'repository', 'recorded_at', 'receipt_type', 'status', 'consumer_repository', 'consumer_commit'], 'consumer review receipt');
  if (consumerReceipt.receipt_type !== 'svc_forum_consumer_migration_review' || consumerReceipt.status !== 'PASS'
      || consumerReceipt.consumer_repository !== 'mayf3/agent-forum' || consumerReceipt.consumer_commit !== FORUM_CONSUMER_COMMIT) fail('consumer review receipt invalid');
  const bundleReceipt = await loadReceipt(receiptLink(bundle.receipt, 'forum_prerequisites.contract_bundle.receipt'), commit, 'bundle validation receipt');
  baseReceipt(bundleReceipt, ['schema_version', 'repository', 'recorded_at', 'receipt_type', 'status', 'contract_version', 'validator_output'], 'bundle validation receipt');
  if (bundleReceipt.receipt_type !== 'minimal_auth_v1_contract_bundle_validation' || bundleReceipt.status !== 'PASS'
      || bundleReceipt.contract_version !== BUNDLE_CONTRACT_VERSION || bundleReceipt.validator_output !== 'MINIMAL_AUTH_V1_BUNDLE_VALID=true') fail('bundle validation receipt invalid');
  return metadata;
}
...
function conformanceUrl(descriptor: Record<string, unknown>): string {
  ...
  if (config.Image !== IMAGE || state.Running !== true || hostConfig.AutoRemove !== true
      || typeof info.Name !== 'string' || !info.Name.startsWith('/auth-stage-f-conformance-')
      || !isObject(labels) || labels['com.mayf3.auth.stage-f-conformance'] !== `sha256:${createHash('sha256').update(nonce).digest('hex')}`
  ...
  return `postgresql://stage_f_runner@127.0.0.1:${descriptor.host_port}/${DATABASE}?schema=public&application_name=stage_f_${nonce}#${Buffer.from(JSON.stringify(privateBinding)).toString('base64url')}`;
}
...
function parseConformanceUrl(value: string): { url: string; metadata: Metadata; nonce: string; serverAddress: string } {
  ...
  const nonce = parsed.searchParams.get('application_name')?.replace(/^stage_f_/, '') ?? '';
...
function validateFixtureEnvelope(kind: string, requestPath: string, body: Record<string, unknown>): void {
  ...
  if (kind === 'auth-commit') {
    if (!/^\/repos\/mayf3\/auth-service\/commits\/(?:main|[0-9a-f]{40})$/.test(requestPath)) fail('auth commit request path invalid');
    lowercaseHex(body.sha, 40, 'fixture auth commit sha');
    return;
  }
  if (kind === 'compare') {
  ...
  if (kind === 'auth-compare') {
    const match = /^\/repos\/mayf3\/auth-service\/compare\/([0-9a-f]{40})\.\.\.([0-9a-f]{40})$/.exec(requestPath);
    if (!match || body.url !== canonicalApi(requestPath) || !isObject(body.base_commit)
        || !isObject(body.merge_base_commit) || body.base_commit.sha !== match[1]
        || body.merge_base_commit.sha !== match[1] || body.behind_by !== 0
        || (body.status !== 'ahead' && body.status !== 'identical')) fail('auth compare fixture envelope invalid');
    return;
  }
...
function validateHttpFixture(value: Json): void {
  ...
  const kinds = ['commit', 'auth-commit', 'compare', 'auth-compare', 'contents-manifest', 'contents-receipt', 'review', 'comment'];
  ...
  const limit = value.kind === 'compare' || value.kind === 'auth-compare' ? 16 * 1024 * 1024
...
async function main(): Promise<void> {
    ...
    try {
      const probe = await prisma.$queryRaw<Array<{ database: string; address: string; port: number; nonce: string }>>`
        SELECT current_database() AS database, inet_server_addr()::text AS address,
               inet_server_port() AS port, current_setting('stage_f.conformance_nonce') AS nonce`;
  ...
  if (args['--validate-evidence'] || args['--apply']) {
    ...
    if (args['--validate-evidence']) { process.stdout.write('STAGE_F_EVIDENCE_VALID=true\n'); return; }
