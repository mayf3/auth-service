...
cleanup() {
  ...
  if [[ -n "$CONTAINER_ID" ]]; then
    ...
    observed_label="$(/usr/local/bin/docker inspect --format '{{index .Config.Labels "com.mayf3.auth.stage-w-conformance"}}' "$CONTAINER_NAME" 2>/dev/null || true)"
  ...
  if [[ -n "$CONTAINER_ID" ]] && /usr/local/bin/docker inspect "$CONTAINER_ID" >/dev/null 2>&1; then
    ...
    remaining_label="$(/usr/local/bin/docker inspect --format '{{index .Config.Labels "com.mayf3.auth.stage-w-conformance"}}' "$CONTAINER_NAME" 2>/dev/null || true)"
  ...
  echo 'STAGE_W_CONFORMANCE_CONTAINER_ABSENT=true'
  exit "$ORIGINAL_EXIT"
}
...
CONTAINER_NAME="auth-stage-w-conformance-$(printf '%s' "$NONCE" | cut -c1-16)"

CONTAINER_ID="$(/usr/local/bin/docker run -d --rm \
  --name "$CONTAINER_NAME" \
  --label "com.mayf3.auth.stage-w-conformance=sha256:$NONCE_HASH" \
...
CREATE ROLE stage_w_runner LOGIN;
GRANT USAGE ON SCHEMA public TO stage_w_runner;
GRANT SELECT (id, client_id, machine_principal_id, external_ref, status) ON machine_clients TO stage_w_runner;
GRANT SELECT ON machine_principals, auth_audiences, trusted_proxies, delegation_grants TO stage_w_runner;
GRANT SELECT, INSERT, UPDATE, DELETE ON machine_access_grants, grant_change_audits TO stage_w_runner;
COMMIT;
SQL

LEGACY_PRIVILEGES="$(/usr/local/bin/docker exec "$CONTAINER_ID" psql -At -U postgres -d "$DATABASE" -c \
  "SELECT has_column_privilege('stage_w_runner','machine_clients','allowed_resources','SELECT')::text || ',' || has_column_privilege('stage_w_runner','machine_clients','allowed_scopes','SELECT')::text")"
[[ "$LEGACY_PRIVILEGES" == 'false,false' ]] || { echo 'FATAL: conformance runner can read legacy columns' >&2; exit 1; }
echo 'STAGE_W_LEGACY_COLUMN_SELECT_PRIVILEGES=false,false'
...
echo 'STAGE_W_AUDIT_CONTROLS_VERIFIED=true'
DESCRIPTOR="$(node -e '
const d={schema_version:1,container_id:process.argv[1],nonce:process.argv[2],host_port:Number(process.argv[3]),database:"auth_stage_w_conformance",audit_metadata:{migration_id:"stage-w-conformance-v1",source_git_commit:"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",operator_id:"stage-w-conformance",approval_ref:"https://github.com/mayf3/auth-service/issues/1#issuecomment-1",reason:"isolated Stage W conformance"}}; process.stdout.write(JSON.stringify(d));
' "$CONTAINER_ID" "$NONCE" "$HOST_PORT")"

(
  cd "$REPO_DIR"
  env -u DATABASE_URL node --import tsx tests/oauth/supply-agentcore-canary-workflow-grants-v1.test.ts \
    3< <(printf '%s' "$DESCRIPTOR")
)

echo 'STAGE_W_TEMP_DB_CONFORMANCE=PASS'
