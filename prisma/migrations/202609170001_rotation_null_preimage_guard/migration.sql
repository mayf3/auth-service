-- ROTATION_NULL_PREIMAGE_GUARD (T87 / AUTH-SCOUT-20260914-C05):
-- The canonical ROTATE seam must reject a NULL preimage fingerprint.
--
...
--   NULL preimage → reject; wrong preimage → reject; correct preimage →
--   rotate. The first two perform ZERO credential mutation.
--
-- DEFECT: the 20260913 function compared the live fingerprint with
--   v_preimage_fp <> lower(p_preimage_fingerprint)
-- Under SQL three-valued logic a NULL p_preimage_fingerprint makes that
-- predicate NULL, the IF treats it as false, the mismatch branch never
-- fires, and the secret rotated with NO preimage proof. The replay-branch
-- live-state invariant already uses IS DISTINCT FROM for exactly this
-- reason; the preimage guard now does the same.
--
-- ADMIN HANDSHAKE (same three steps as 20260909010000/20260913; the CREATE
-- OR REPLACE below must run as/with the function owner machine_credential_owner):
...
BEGIN
  ...
  -- T87: IS DISTINCT FROM — a NULL preimage fingerprint is a MISMATCH, never
  -- a pass-through (three-valued <> evaluated to NULL and skipped the guard).
  ...
  IF v_preimage_fp IS DISTINCT FROM lower(p_preimage_fingerprint) THEN
